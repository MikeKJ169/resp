/**
 tinymce plugin to upload images using Django
 */

tinyMCE.importPluginLanguagePack('djangoimage2', 'en');
...
// Singleton class
var TinyMCE_DjangoImage2Plugin = {
    /**
    ...
    getInfo : function() {
        return {
            longname : 'DjangoImage2 plugin',
            author : 'Marcus Harris',
            authorurl : 'http://www.paston.co.uk',
            infourl : 'http://www.paston.co.uk',
            version : "2.0"
        };
    },
...
//        alert("Initialization parameter:" + tinyMCE.getParam("djangoimage2_someparam", false));

        // Register custom keyboard shortcut
//        inst.addShortcut('ctrl', 't', 'lang_djangoimage2_desc', 'mceDjangoImage2');

        editor_timestampfield=document.getElementById('id_editor_timestamp');
        editor_timestampfield.value=new Date().getTime();
        editor_idfield=document.getElementById('id_editor_uniqid');
        editor_idfield.value=TinyMCE_DjangoImage2Plugin._uniq();
    },
    ...
    getControlHTML : function(cn) {
        switch (cn) {
            case "djangoimage2":
                return tinyMCE.getButtonHTML(cn, 'lang_djangoimage2_desc', '{$pluginurl}/images/djangoimage.gif', 'mceDjangoImage2', true);
        }
    ...
    execCommand : function(editor_id, element, command, user_interface, value) {
        ...
        switch (command) {
            ...
            case "mceDjangoImage2":
                // Show UI/Popup
                if (user_interface) {
                    editor_timestampfield=document.getElementById('id_editor_timestamp');
                    editor_timestamp=editor_timestampfield.value;
                    editor_uniqidfield=document.getElementById('id_editor_uniqid');
                    editor_uniqid=editor_uniqidfield.value;

			        var djangoimage2 = new Array();

//                    djangoimage2['file'] = '/admin/smartpages/image/add/?_popup=1&editor_timestamp='+editor_timestamp+'&editor_uniqid='+editor_uniqid;
                    djangoimage2['file'] = '/admin/smartpages/image/add/?_popup=1&editor_timestamp='+editor_timestamp+'&editor_uniqid='+editor_uniqid;
                    djangoimage2['width'] = 740;
                    djangoimage2['width'] = 800;
                    djangoimage2['height'] = 190;

                    tinyMCE.openWindow(djangoimage2, {editor_id : editor_id, some_custom_arg : "somecustomdata"});

                    // Let TinyMCE know that something was modified
                    tinyMCE.triggerNodeChange(false);
                } else {
                    // Do a command this gets called from the djangoimage2 popup
//                    alert("execCommand: mceDjangoImage2 gets called from popup.");
                }

                return true;
            case "mceDjangoInsertImageHtml":
                html='<img src="'+value+'" />';
...
//        // Select djangoimage button if parent node is a strong or b
//        if (node.parentNode.nodeName == "STRONG" || node.parentNode.nodeName == "B") {
//            tinyMCE.switchClass(editor_id + '_djangoimage', 'mceButtonSelected');
//            return true;
//        }
//
//        // Deselect djangoimage button
//        tinyMCE.switchClass(editor_id + '_djangoimage', 'mceButtonNormal');
//    },
...
//        top.status = "djangoimage plugin event: " + e,type;
//
...
//    }
    ...
    _uniq : function() {
        uniq='';
        for (i=0;i<4;i++) {
            uniq+=String.fromCharCode(65 + Math.round(Math.random() * 25));
        }
        return uniq;
    }
...
tinyMCE.addPlugin("djangoimage2", TinyMCE_DjangoImage2Plugin);
